    ...
    public function _import($tag, $content, $isFile = false, $type = '')
    {
        ...
        if ($isFile) {
            ...
            foreach ($array as $val) {
                ...
                switch ($type) {
                    case 'js':
                        $parseStr .= '<script type="text/javascript" src="'.$basepath.'/'.str_replace(
                                ['.', '#'],
                                ['/', '.'],
                                $val
                            ).'.js'.($version ? '?'.$version : '').'"></script>';
                        break;
                    case 'css':
                        $parseStr .= '<link rel="stylesheet" type="text/css" href="'.$basepath.'/'.str_replace(
                                ['.', '#'],
                                ['/', '.'],
                                $val
                            ).'.css'.($version ? '?'.$version : '').'" />';
