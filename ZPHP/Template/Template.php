...
class Template
{
    ...
    public function parse($content)
    {
        ...
        $content = preg_replace_callback(
            '/('.$this->config['tmpl_begin'].')([^\d\w\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/is',
            [$this, 'parseTag'],
            $content
        );
    ...
    public function parseTag($tagStr)
    {
        ...
        if ('$' == $flag && '.' != $flag2 && '(' != $flag2) { //解析模板变量 格式 {$varName}
            ...
            return '<?php '.$name.';?>';
        } elseif (substr($tagStr, 0, 2) == '//' || (substr($tagStr, 0, 2) == '/*' && substr(
                    rtrim($tagStr),
                    -2
                ) == '*/')
        ) {
    ...
    public function parseVar($varStr)
    {
        ...
        if (!empty($varStr)) {
            ...
            if (false !== strpos($var, '.')) {
                ...
                $var = $match[1];
            } elseif (false !== strpos($var, ':') && false === strpos($var, '(') && false === strpos(
                    $var,
                    '::'
                ) && false === strpos($var, '?')
            ) {
